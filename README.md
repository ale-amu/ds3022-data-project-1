...
- **YELLOW:** 37.947 kg CO2, 99.86 miles, picked up at 2024-10-27 01:28:18
- **GREEN:** 34.748 kg CO2, 99.28 miles, picked up at 2024-02-28 11:11:12
...
- **YELLOW:** Heaviest = hour 6 (2.320 kg avg/trip); Lightest = hour 19 (1.142 kg avg/trip)
- **GREEN:** Heaviest = hour 6 (1.597 kg avg/trip); Lightest = hour 19 (0.918 kg avg/trip)
...
- **YELLOW:** Heaviest = Sunday (1.462 kg avg/trip); Lightest = Saturday (1.219 kg avg/trip)
- **GREEN:** Heaviest = Sunday (1.121 kg avg/trip); Lightest = Tuesday (1.004 kg avg/trip)
...
- **YELLOW:** Heaviest = week 35 (1.474 kg avg/trip); Lightest = week 51 (1.170 kg avg/trip)
- **GREEN:** Heaviest = week 35 (1.394 kg avg/trip); Lightest = week 3 (0.944 kg avg/trip)
...
- **YELLOW:** Heaviest = August (1.396 kg avg/trip); Lightest = February (1.226 kg avg/trip)
- **GREEN:** Heaviest = August (1.147 kg avg/trip); Lightest = January (0.969 kg avg/trip)
...
## how to run 
### This lab has scripts that all depend on the one run prior to it. Within the lab, every pyython file has its own log file eg.(load.log, clean.log, transform.log, analysis.log)
...
For CO2 vehicle emissoons, I realized that the assignment instructed that we connect this vaue to the look up table, so it was not meant to be a set figure in our code so I decide to look it up by vehicle type instead of hard coding it. I also decided to list my hours as HOUR() +1 because during my initial run I realized that duck DB's hour returned 0-23 even though the assignment asked for us to use a 1-24 scale This is why I added the +1 shift. Lastly, I used two separate Y axes for the monthly plot because I realized that yellow has about 60 times more trips compared to green so when on one one shared scale Green's line looked flat like a 0